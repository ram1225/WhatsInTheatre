...
export class HttpService {
  private TRENDING_MOVIES = 'https://api.themoviedb.org/3/trending/movie/week?api_key=6b28e739331f7d86bcd6e5e8e7d9cb08';
  private IMAGE_BASE_URL = 'http://image.tmdb.org/t/p/w500/';
  results: any[];
  constructor(private http: HttpClient) { }

  public get(): Observable<any> {
    return this.http.get(this.TRENDING_MOVIES).pipe(
      map(res => {
        return res['results'].map(trendingmovie => {
          return new TrendingMovie(
            trendingmovie.adult,
            this.IMAGE_BASE_URL + trendingmovie.backdrop_path,
